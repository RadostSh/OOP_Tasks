...
{
    str = str.substr(1, str.size() - 2); // removes the first and last brackets
    ...
    for (size_t i = 0; i < str.size(); i++) {
        if (str[i] == '(') {
            ...
            switch (str[i]) {
                case '!': return new Negation(expressionFactory(str.substr(i + 1, str.size() - i - 1)));
                case '&': return new Conjunction(expressionFactory(str.substr(0, i)), expressionFactory(str.substr(i + 1, str.size() - i - 1)));
                case '|': return new Disjunction(expressionFactory(str.substr(0, i)), expressionFactory(str.substr(i + 1, str.size() - i - 1)));
                case '>': return new Implies(expressionFactory(str.substr(0, i)), expressionFactory(str.substr(i + 1, str.size() - i - 1)));
            default: ;
            }
        }
    }
